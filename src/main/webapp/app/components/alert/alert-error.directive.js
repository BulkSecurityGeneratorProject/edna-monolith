(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('ednaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-ednaApp-error');
                var entityKey = httpResponse.headers('X-ednaApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('ednaApp.' + fieldError.objectName + '.' + convertedField);
