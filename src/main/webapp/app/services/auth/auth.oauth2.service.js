    ...
    function AuthServerProvider ($http, $localStorage, Base64) {
        ...
        function login (credentials) {
            var data = 'username=' +  encodeURIComponent(credentials.username) + '&password=' +
                ...
                'client_secret=my-secret-token-to-change-in-production&client_id=ednaapp';

            return $http.post('oauth/token', data, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'Accept': 'application/json',
                    'Authorization': 'Basic ' + Base64.encode('ednaapp' + ':' + 'my-secret-token-to-change-in-production')
                }
            }).success(authSucess);
